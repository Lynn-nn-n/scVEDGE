...
# ######################################################################################################################
adata = anndata.read("../Data/dogma_all_genes_cells_75unpaired.h5ad.gz")
VEDGE.setup_anndata(adata, batch_key='modality' , protein_expression_obsm_key='protein_expression', categorical_covariate_keys=['sample_type'])
torch.set_float32_matmul_precision('high')
#adata.var['modality'] = adata.var['Type'].copy()
n_genes = (adata.var.modality=='Gene Expression').sum()
...
mvi = VEDGE(adata, n_genes=n_genes, n_regions=n_regions)
mvi.view_anndata_setup()
testing(mvi, save_path="../model_trained/Test3Mod_75unpaired", pdf_path="../Data/dogma_all_genes_cells_75unpaired")
# ######################################################################################################################
adata = anndata.read("../Data/dogma_all_genes_cells_allcomb.h5ad.gz")
...
mvi = VEDGE(adata, n_genes=n_genes, n_regions=n_regions)
mvi.view_anndata_setup()
testing(mvi, save_path="../model_trained/Test3Mod_all7", pdf_path="../Data/dogma_all_genes_cells_allcomb")
# ######################################################################################################################