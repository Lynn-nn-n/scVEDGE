...
class VEDGE_NET(BaseModuleClass):
    ...
    def __init__(
        ...
        use_spectral_norm: bool = True, #True,
        ...
        #print("n_hidden",self.n_hidden) 261
        ...
        # print("encoder here:",n_input_encoder_exp,n_latent)
        ...
        if self.n_input_genes>0:
            self.discriminator_rna = Discriminator(
            n_input_genes,
            1,
            n_layers = self.n_layers_encoder,
            n_hidden = self.n_hidden,
        ...
        if self.n_input_regions>0:
            self.discriminator_atac = Discriminator(
            n_input_regions,
            1,
            n_layers = self.n_layers_encoder,
            n_hidden = self.n_hidden,
        ...
        if self.n_input_proteins>0:
            ...
            self.discriminator_pro = Discriminator(
    ...
    def inference(
       ...
       # Z Encoders
        ...
        outputs = {
            ...
            'match_exp2acc':match_exp2acc,
            'unmatch_exp2acc':unmatch_exp2acc,
        }
        return outputs
    ...
    def _get_generative_input(self, tensors, inference_outputs, transform_batch=None):
        ...
        z_rna = inference_outputs["z_expr"]
        qz_m_rna = inference_outputs["qzm_expr"]
        z_atac = inference_outputs["z_acc"]
        qz_m_atac = inference_outputs["qzm_acc"]
    ...
    def generative(
        ...
        z_p=Normal(0, 1).sample(z.size()).to(z.device)
        ...
        latent_exp = z_exp if not use_z_mean else qz_m_exp
        latent_acc = z_acc if not use_z_mean else qz_m_acc
       # latent_exp = z_
        if cont_covs is None:
            decoder_input = latent
            decoder_input_exp = latent_exp
        ...
        # Accessibility Decoder
        p_atac = self.z_decoder_accessibility(decoder_input, batch_index, *categorical_input)
        xp_atac = self.z_decoder_accessibility(z_p, batch_index, *categorical_input)
        #p_atac_from_exp = self.z_decoder_accessibility(decoder_input_exp, batch_index, *categorical_input)
        ...
        px_scale_, _, px_rate_, px_dropout_ = self.z_decoder_expression(
            self.gene_dispersion,
            z_p,
            size_factor,
            batch_index,
        ...
        py_2, log_pro_back_mean2 = self.z_decoder_pro(z_p, batch_index, *categorical_input)
        # Protein Dispersion
        if self.protein_dispersion == "protein-label":
            # py_r gets transposed - last dimension is n_proteins
            py_r = F.linear(F.one_hot(label.squeeze(-1), self.n_labels).float(), self.py_r)
        ...
        p_pro = py_conditional.rsample()
        py_conditional2 = NegativeBinomialMixture(
        mu1=py_2["rate_back"],
        mu2=py_2["rate_fore"],
        theta1=py_2["r"],
        mixture_logits=py_2["mixing"],
    )
        xp_pro = py_conditional2.rsample()

        f1p,_=self.discriminator_atac(p_atac) if self.discriminator_atac is not False else (None,None)
        f2p,_=self.discriminator_rna(p_rna) if self.discriminator_rna is not False else (None,None)
        f1xp,_=self.discriminator_atac(xp_atac) if self.discriminator_atac is not False else (None,None)
        f2xp,_=self.discriminator_rna(xp_rna) if self.discriminator_rna is not False else (None,None)
        f3p,_=self.discriminator_pro(p_pro) if self.discriminator_pro is not False else (None,None)
        f3xp,_=self.discriminator_pro(xp_pro) if self.discriminator_pro is not False else (None,None)
        ...
        return {
            ...
            #'p_rna_from_acc':p_rna_from_acc,
            #'p_atac_from_exp':p_atac_from_exp,
        }
    ...
    def loss(self, tensors, inference_outputs, generative_outputs, kl_weight: float = 1.0):
        ...
        loss = torch.mean(recon_loss 
                          #+ rl_fD_atac +rl_fD_rna 
                          + weighted_kl_local*self.lambd1)
        #loss = torch.mean(recon_loss + weighted_kl_local*self.lambd1)
